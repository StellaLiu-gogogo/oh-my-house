...
## 19.25 Meal 卡路里与实际食用比例

卡路里作为 Meals / Recipes 的可选轻量记录，不扩展成减肥、体重或完整营养管理功能。

预存食谱自带“默认份数对应的整份食谱估算卡路里”。使用预存食谱安排 Meal 时，App 根据本次实际制作
份数计算整顿饭的估算卡路里：

``` text
本次整顿饭估算卡路里
= 预存食谱整份卡路里 × 本次制作份数 ÷ 食谱默认份数
```

每位参与成员可以填写自己吃掉“本次实际制作的全部餐食”的百分比，App 再计算该成员本次摄入：
...
界面必须明确说明百分比指整顿饭的全部成品，而不是该成员自己盘子里的完成比例。提供 25%、50%、75%、
100% 快速选择和自定义百分比；未被成员记录的比例可以作为剩余或未记录内容，不强制分配。

用户自己创建的 Recipe 和没有关联预存食谱的普通 Meal 不自动猜测卡路里。用户可以选择手动填写整份食谱
或本次 Meal 的估算卡路里，再使用同一套百分比折算；未填写时仍可正常保存和使用。

第一阶段只记录 kcal，不记录或分析蛋白质、脂肪、碳水、糖、盐、维生素等营养成分，也不提供每日目标、
减肥建议、体重记录、健康判断或医学级准确性声明。所有数值统一显示为“估算卡路里”。
