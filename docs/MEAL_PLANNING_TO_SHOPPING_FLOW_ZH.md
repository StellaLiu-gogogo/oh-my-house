...
**Agent 建议：** 默认选择全部家庭成员，用户只在有人不吃时调整。
...
**Agent 建议：** 使用从周一到周日的纵向列表，每天显示 Lunch / Dinner；不在 iPhone 上使用七列横向表格。

### 6.2 餐食创建方式

**Agent 建议：** 同时支持“只写餐食名称”“选择 Recipe”“重复最近餐食”三种方式。

### 6.3 默认参与成员

**Agent 建议：** 新增餐食时默认选择所有家庭成员，需要时再取消某位成员。

### 6.4 食材进入 Shopping

**Agent 建议：** Recipe ingredients 必须经过一次简单确认后才能加入 Shopping，不自动加入。

### 6.5 简单餐食不猜测 ingredients

**Agent 建议：** 第一阶段不根据“咖喱饭”“煮面”等自由文本自动猜测需要购买的食材。
