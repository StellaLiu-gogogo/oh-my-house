...
- **状态：** 本地保存已通过；模拟器不支持 Apple Translation，等待真实 iPhone 确认
...
Apple Translation 需要在真实 iPhone 的运行界面里触发。首次使用时，系统可能要求下载中文或英文语言包。
用户需要使用自己的 Apple Account，通过 Xcode 的 Personal Team 把测试程序安装到自己的 iPhone，
再点击一次“开始翻译”，确认：
