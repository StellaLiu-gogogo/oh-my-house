...
以下条件仍是完整 App（包含 iCloud 共享）最终通过标准：
...
用户已决定分阶段进行：本地保存与 Apple Translation 通过后，可以先开发不包含 iCloud 的正式 App 主体；
第 2–6 项未验证前，不显示或承诺同步、邀请、多人共享和云端头像能力。
...
1. 本地保存与 Apple Translation 已验证通过；
2. 开始制作本地优先的正式 App 主体；
3. 暂时隐藏 iCloud、邀请和已连接使用者入口；
4. 数据结构和统一数据管理入口为未来 CloudKit 保留接入位置；
5. 用户以后决定加入 Apple Developer Program 时，再验证 iCloud、多人共享和离线冲突。
