...
- 允许参与者继续邀请其他人：iOS / iPadOS / macOS 26；
- Apple Translation 框架：iOS / iPadOS 17.4，macOS 14.4。

“所有已连接使用者都可以邀请和移除其他使用者”是已确认的需求。
如果不想修改它，最简单、最可靠的方案是：

> Oh My House 最低支持 iOS 26、iPadOS 26 和 macOS 26。

这不是为了追求最新界面，而是为了使用符合已确认家庭权限规则的 CloudKit 能力。

### 当前设备情况

当前用于项目的 Mac 是 M2 MacBook Air，目前运行 macOS 15.7.8。Apple 确认这个型号可以升级至 macOS 26。

在确认最低版本前，还需要知道实际使用的 iPhone 和 iPad 型号，确认它们能否运行系统 26。

## 6. 为什么不先兼容更旧的系统

如果把最低版本降到 iOS 18 / macOS 15：

- 你仍然可以创建一次性邀请；
- 旧设备也可以加入和编辑日常内容；
- 但“所有连接成员共同管理访问”需要变成只由底层所有者管理，或自己再做一套不够简单的补充机制。

这个 App 是家庭内部使用，不是需要覆盖大量旧设备的商业 App。
如果家里的实际设备都支持系统 26，优先保持规则简单更合理。
...
推荐：如果家里实际使用的 iPhone 和 iPad 都可以运行系统 26，则接受。

用户只需要提供计划使用的 iPhone 和 iPad 型号，不需要自己研究兼容性。

### 决定 C：是否先做可行性验证

推荐：接受，但只在 A 和 B 都确认后才开始。

可行性验证通过后，才制作正式 App 结构。
